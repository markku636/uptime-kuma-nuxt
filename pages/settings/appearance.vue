<template>
  <div class="section-card bg-gray-50 dark:bg-gray-800">
    <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-6">Appearance</h4>
    ...
    <form @submit.prevent="saveAppearance" class="space-y-6">
      ...
      <div>
        ...
        <div class="flex gap-4">
          <button v-for="t in themeOptions" :key="t.value" type="button"
            ...
            :class="theme === t.value ? 'border-green-500 bg-gray-200 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'"
      ...
      <div>
        ...
        <div class="flex gap-3 flex-wrap">
          <button v-for="color in colorOptions" :key="color.value" type="button"
            ...
            :class="primaryColor === color.value ? 'border-gray-900 dark:border-white scale-110' : 'border-transparent'"
      ...
      <!-- Uptime Display -->
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Uptime Display Style</label>
        <div class="space-y-2">
          <label class="flex items-center gap-2 cursor-pointer">
            <input v-model="uptimeDisplayStyle" type="radio" name="uptimeStyle" value="percentage" />
            <span class="text-gray-700 dark:text-gray-300">Percentage (99.99%)</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
            <input v-model="uptimeDisplayStyle" type="radio" name="uptimeStyle" value="nines" />
            <span class="text-gray-700 dark:text-gray-300">Nines (4 nines)</span>
          </label>
        </div>
      </div>
      ...
      <!-- Heartbeat Bar Style -->
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Heartbeat Bar Style</label>
        <div class="flex gap-4">
          <button v-for="style in heartbeatBarOptions" :key="style.value" type="button"
            class="flex flex-col items-center gap-2 p-4 rounded-lg border-2 transition-all"
            :class="heartbeatBarStyle === style.value ? 'border-green-500 bg-gray-200 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'"
            @click="heartbeatBarStyle = style.value">
            <span class="text-gray-700 dark:text-gray-300">{{ style.label }}</span>
          </button>
        </div>
      </div>
      ...
      <!-- Elapsed Time Style -->
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Elapsed Time Style</label>
        <div class="flex gap-4 flex-wrap">
          <button v-for="style in elapsedTimeOptions" :key="style.value" type="button"
            ...
            :class="elapsedTimeStyle === style.value ? 'border-green-500 bg-gray-200 dark:bg-gray-700' : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'"
            @click="elapsedTimeStyle = style.value">
            <span class="text-gray-700 dark:text-gray-300 text-sm">{{ style.label }}</span>
          </button>
        </div>
      </div>
      ...
      <div>
        <label class="flex items-center gap-2 cursor-pointer">
          ...
          <span class="text-gray-700 dark:text-gray-300">Show Certificate Expiry Days</span>
        </label>
      </div>
